...
def process_calibration():
    ...
    while True:
        if not data_queue.empty():
            ...
            try:
                # print(f"Inserting calibration data @ {data[0]['coordinates']}")
...
#         if not data_queue.empty():
#             data = data_queue.get()  # Dequeue a frame
#             # images_bytes = base64.b64decode(data[1])
#             try:
#                 print(f"Inserting calibration data @ ok")
#                 insert_to_cdb(data[0], data[1])
#             except Exception as e:
#                 logger.error(f"Error processing frame: {e}")
#             finally:
#                 data_queue.task_done()  # Mark the frame as processed
